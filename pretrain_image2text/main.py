...
def main(parent_parser):
    ...
    if args.stage == "predict":
        ...
        # end of training
        if args.use_nni and trainer.is_global_zero:
            logging.info(f'Rank{trainer.global_rank}: Running final report for nni')
...
def get_params():
    ...
    parser.add_argument("--precision", type=int, default=32, help='training precision', choices=[32, 16, 8])
    parser.add_argument("--gradient_clip_val", type=float, default=0., help='gradient clip val. 0 means no clip')
    parser.add_argument("--save_top_k", type=int, default=1, help='save top k best models')
