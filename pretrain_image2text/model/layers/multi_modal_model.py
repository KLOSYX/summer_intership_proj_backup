...
class VisualEncoderMLMEncoder(nn.Module):
    ...
    def forward_text(self, text_inputs, is_mlm=True):
        # loss for mlm
        outputs = self.text_decoder(input_ids=text_inputs.input_ids,
                                    attention_mask=text_inputs.attention_mask,
                                    encoder_hidden_states=None,
                                    encoder_attention_mask=None,
                                    labels=text_inputs.labels if is_mlm else None,
                                    return_dict=True,
                                    output_hidden_states=True,
                                    mode='text')  
        last_hidden_state = outputs.hidden_states[-1] # (N, L, dim)
        
        return (outputs.loss, last_hidden_state) if is_mlm else last_hidden_state[:, 0, :]
