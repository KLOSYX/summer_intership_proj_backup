...
class FinetuneVted(pl.LightningModule):
    def __init__(self, 
                 tokenizer,
        ...
        self.model = VisualEncoderLMDecoder(visual_encoder=visual_encoder, text_decoder=text_decoder, tokenizer=tokenizer)
    ...
    def validation_step(self, batch, batch_idx):
        pixels, tokens = batch
        loss = self(pixels, tokens)
        generate_ids = self.model.generate(pixels, num_beams=3, max_length=64, min_length=5, no_repeat_ngram_size=2)
        ...
        # if self.hparams.stage == 'fit' and self.global_rank == 0:
        ...
        #     else:
        #         columns = ['preds', 'image']
        #         data = [[pred, wandb.Image(rearrange(pixels[i, :, :, :], 'c h w -> h w c'), caption=f'{target[0]}')] for i, (pred, target) in enumerate(zip(preds, targets))]
