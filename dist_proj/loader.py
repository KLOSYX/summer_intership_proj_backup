...
def load_model_path(project_name, root=None, version=None, v_num=None, best=None):
    ...
    def sort_by_epoch(path):
        name = path.stem
        epoch = float(name.split('-')[-1].split('=')[1])
        return epoch
    ...
    def generate_root():
        if root is not None:
            ...
            return str(Path('pl_log', project_name, version, 'checkpoints'))
        else:
            return str(Path('pl_log', project_name, f'version_{v_num}', 'checkpoints'))
    ...
    if Path(root).is_file():
        return root
    if best:
        files = [i for i in list(Path(root).iterdir())
                 if i.stem.startswith('best')]
...
def load_model_path_by_args(args):
    return load_model_path(project_name=args.project_name,
                           root=args.load_dir,
